...
const Shop = () => {
  ...
  const handleFilter = (a) => {
    ...
    if (filterValue === "Coffee") {
      const filterDrinks = Drinks.filter((item) => item.category === "Coffee");
      setDrinksData(filterDrinks);
    }

    if (filterValue === "Tea") {
      const filterDrinks = Drinks.filter((item) => item.category === "Tea");
      setDrinksData(filterDrinks);
    }
  ...
  const handleSearch = (b) => {
    ...
    const searchedDrinks = Drinks.filter((item) =>
          ...
          <Row>
            ...
            <Col lg="5" md="8">
              <div className="search_box">
                <input
                  type="text"
                  placeholder="Find your lovely drinks.."
          ...
          <Row>
            {drinksData.length === 0 ? (
              <h1 className="text-center fs-6">
                {" "}
                you haven't chosen a drink yet{" "}
              </h1>
            ) : (
